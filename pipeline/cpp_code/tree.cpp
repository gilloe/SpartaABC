...
//A.M. I'm guessing it's the opposite of "remove son"
...
MDOUBLE tree::TreeNode::getMinimalDistance2OTU(){	
	if(this->isLeaf())
		...
		{
		case 0:
			LOG(3, <<"ERROR: number of sons for node is zero, but not return leaf\n");
			return -1;
			break;
		case 1:
		...
		case 4:
			...
			break;
		default:
			LOG(3, <<"ERROR: number of sons for node "<< numberOfSons<<" is not implemented in getMinimalDistance2OTU\n");
			return -1;
		}
...
int tree::TreeNode::getMinimalNumOfNodes2OTU(){	
	...
	if(this->isLeaf())
		...
		case 4:
			...
			break;
		default:
			LOG(3, <<"ERROR: number of sons for node "<< numberOfSons<<" is not implemented in getMinimalNumOfNodes2OTU\n");
			return -1;
		}
	}
}
...
void tree::getNeigboursOfNode(vector<nodeP> &vNeighbourVector, const nodeP inNodeP) const {
	...
	for (size_t i=0; i < inNodeP->getNumberOfSons();++i) {
...
MDOUBLE tree::findLengthBetweenAnyTwoNodes(const nodeP node1, const nodeP node2) const {
	...
	for (size_t i=0; i < pathMatrix.size() ; i++) {
...
void tree::getTreeDistanceTableAndNames(VVdouble& disTab, vector <string>& vNames) const {
  ...
  for (size_t i=0;i<nodepV.size();++i) {
    disTab[i].resize(nodepV.size());
    vNames[i]=nodepV[i]->name();
    for(size_t j=i+1;j<nodepV.size();++j){
...
void tree::rootAt(const nodeP p_iNewRoot) {
	...
	for (size_t i = 0; i < pathMatrix.size() - 1 ; i++) {
	...
	for (size_t i=0; i < _nodevec.size(); ++i) {
		if (_nodevec[i]!=_root) {
			if (_nodevec[i]->dis2father()<epsilon) {
				LOG(4,<<" @@@ Warning: brachLength too short:"<<endl
...
void tree::outputInAncestorTreeFormat(ostream& treeOutStream, bool distances) const{
	...
	//WAS A PROBLEM: CERROR:4669 => treeOutStream<<"# created on "<< ctime( &ltime ) ;
	ctime_s(time_str, SIZE, &ltime);
	treeOutStream << "# created on " << time_str;

	treeOutStream<<"name";

	// let's say that the max length of a name of a species is 7. So after "name" we put 3 spaces.
	if (maxNameLen <= 4) {
		spaces = 1;
	}
	else {
		if (spaces>4) spaces = maxNameLen - 4;
		else spaces = 0;
	}
	
	for (size_t k=0;k<spaces;++k) treeOutStream<<" ";
	...
	for (size_t i=0; i<vec.size();++i) {
		...
		if ((maxNameLen - vec[i]->name().size())>0) 
		...
		for (size_t j=0; j < vec[i]->getNumberOfSons(); j++) {
...
void tree::shrinkNode(nodeP nodePTR) {
	...
	if( (nodePTR->isRoot())&&(nodePTR->getNumberOfSons() == 1) ) // refering the root to be sonNode.
	{
		...
		for (size_t i=0; i < sonNode->getNumberOfSons(); ++i)
		{
...
void tree::rootToUnrootedTree() {
	...
	if (getRoot()->getSon(0)->getNumberOfSons() == 0) {
		...
		for (size_t k = 1; k < toRemove->getNumberOfSons(); ++k) {
