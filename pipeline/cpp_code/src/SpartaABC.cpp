...
int main(int argc, const char * argv[])
{
	...
	while(totalNumberOfSimulations < totalNumberOfSpartaTests)
	{
		...
		double randADeltetionParam = getRandDoubleParamVal(SpartaABC_options::_priorDistTypeA, SpartaABC_options::_minAVal, SpartaABC_options::_maxAVal);
		...
		if (!model_choice[0]){
			randADeltetionParam = randAInsertionParam;
		}
		if (!model_choice[1]){
